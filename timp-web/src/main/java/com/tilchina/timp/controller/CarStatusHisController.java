...
*/
@RestController
@RequestMapping(value = {"/s1/carstatushis"}, produces = {"application/json;charset=utf-8"})
@Slf4j
public class CarStatusHisController extends BaseControllerImpl<CarStatusHis>{

	@Autowired
	private CarStatusHisService carstatushisservice;
	
	@Override
	protected BaseService<CarStatusHis> getService() {
		return carstatushisservice;
	}
    ...
    public ApiResult<String> delete(@RequestBody ApiParam<Long> param) {
        ...
        try {
            carstatushisservice.deleteById(param.getData());
    ...
    public ApiResult<String> deleteList(@RequestBody ApiParam<int[]> param) {
        ...
        try {
        	...
        	carstatushisservice.deleteList(param.getData());
	...
	@Auth(ClientType.WEB)
    public ApiResult<String> putPartList(@RequestBody ApiParam<List<CarStatusHis>> param) {
        log.debug("put: {}", param);
        try {
            
        	carstatushisservice.update(param.getData());
            return ApiResult.success(LanguageUtil.getMessage("0000"));
        } catch (Exception e) {
            log.error(LanguageUtil.getMessage("9999"),"param={}", param, e);
            return ApiResult.failure("9999", e.getMessage());
    ...
    @Override
    @Auth(ClientType.WEB)
    public ApiResult<CarStatusHis> getById(@RequestBody ApiParam<Long> param) {
        log.debug("get: {}", param);
        try {
        	CarStatusHis t = getService().queryById(param.getData());
    ...
    @Override
    @Auth(ClientType.WEB)
    public ApiResult<List<CarStatusHis>> getList(@RequestBody ApiParam<Map<String, Object>> param) {
        log.debug("getList: {}", param);
        try {
            PageInfo<CarStatusHis> pageInfo = getService().queryList(param.getData(), param.getPageNum(), param.getPageSize());
    ...
    public ApiResult<String> post(@RequestBody ApiParam<String> param) {
        ...
        try {
            ...
            CarStatusHis t = (CarStatusHis)JSON.parseObject(param.getData(), clazz);
            getService().add(t);
            return ApiResult.success(LanguageUtil.getMessage("0000"));
        } catch (Exception e) {
