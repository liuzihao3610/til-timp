...
*/
@RestController
@RequestMapping(value = {"/s1/trailer"}, produces = {"application/json;charset=utf-8"})
@Slf4j
public class TrailerController extends BaseControllerImpl<Trailer>{

	@Autowired
	private TrailerService trailerservice;
	
	@Override
	protected BaseService<Trailer> getService() {
    ...
    public ApiResult<String> delete(@RequestBody ApiParam<Long> param) {
        ...
        try {
            trailerservice.deleteById(param.getData());
    ...
    public ApiResult<String> deleteList(@RequestBody ApiParam<int[]> param) {
        ...
        try {
        	...
        	trailerservice.deleteList(param.getData());
            return ApiResult.success(LanguageUtil.getMessage("0000"));
        } catch (Exception e) {
	...
	@Auth(ClientType.WEB)
    public ApiResult<String> putPartList(@RequestBody ApiParam<List<Trailer>> param) {
        log.debug("put: {}", param);
        try {
            
        	trailerservice.update(param.getData());
            return ApiResult.success(LanguageUtil.getMessage("0000"));
        } catch (Exception e) {
            log.error(LanguageUtil.getMessage("9999"),"param={}", param, e);
            return ApiResult.failure("9999", e.getMessage());
    ...
    public ApiResult<String> audit(@RequestBody ApiParam<Long> param) {
        log.debug("put: {}", param);
        try {
        	trailerservice.audit(param.getData());
            return ApiResult.success(LanguageUtil.getMessage("0000"));
        } catch (Exception e) {
            log.error(LanguageUtil.getMessage("9999"),"param={}", param, e);
            return ApiResult.failure("9999", e.getMessage());
    ...
    public ApiResult<String> unaudit(@RequestBody ApiParam<Long> param) {
        log.debug("put: {}", param);
        try {
        	trailerservice.unaudit(param.getData());
            return ApiResult.success(LanguageUtil.getMessage("0000"));
        } catch (Exception e) {
            log.error(LanguageUtil.getMessage("9999"),"param={}", param, e);
            return ApiResult.failure("9999", e.getMessage());
	...
	@Auth(ClientType.WEB)
    public ApiResult<List<Trailer>> getReferenceList(@RequestBody ApiParam<Map<String, Object>> param) {
        log.debug("put: {}",param);
        try {
            
        	PageInfo<Trailer> pageInfo =trailerservice.getReferenceList(param.getData(),param.getPageNum(),param.getPageSize());
    ...
    @Override
    @Auth(ClientType.WEB)
    public ApiResult<Trailer> getById(@RequestBody ApiParam<Long> param) {
        log.debug("get: {}", param);
        try {
        	Trailer t = getService().queryById(param.getData());
    ...
    @Override
    @Auth(ClientType.WEB)
    public ApiResult<List<Trailer>> getList(@RequestBody ApiParam<Map<String, Object>> param) {
        log.debug("getList: {}", param);
        try {
            PageInfo<Trailer> pageInfo = getService().queryList(param.getData(), param.getPageNum(), param.getPageSize());
    ...
    public ApiResult<String> post(@RequestBody ApiParam<String> param) {
        ...
        try {
            ...
            Trailer t = (Trailer)JSON.parseObject(param.getData(), clazz);
            getService().add(t);
            return ApiResult.success(LanguageUtil.getMessage("0000"));
        } catch (Exception e) {
    ...
    public ApiResult<String> importExcel(@RequestParam("file") MultipartFile file) {
        try {
            trailerservice.importExcel(file);
    ...
    public ApiResult<String> exportExcel(HttpServletResponse response) {
        try {
            Workbook workbook = trailerservice.exportExcel();
            ExcelUtil.returnToBrowser(response, workbook, "挂车型号档案");
            return ApiResult.success();
        } catch (Exception e) {
            log.error("{}", e);
            return ApiResult.failure("9999", e.getMessage());
